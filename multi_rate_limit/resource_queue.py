...
class PastResourceQueue:
  ...
  def search_time_after(self, time: float) -> int:
    return bisect.bisect_right(self.time_resource_queue, time, key=lambda t: t[0])

  def search_accum_resouce_within(self, order: int, amount: int) -> int:
  ...
  def add(self, use_time: float, use_resources: List[int]):
    ...
    pos = self.search_time_after(use_time - self.longest_period_in_seconds)
...
class CurrentResourceBuffer:
  ...
  def start_coroutine(self, use_resources: List[int], coro: Coroutine[Tuple[Optional[List[int]], Any]]
      , future: Future[Any]) -> Optional[Task[Tuple[Optional[Tuple[float, List[int]]], Any]]]:
    if self.is_full():
      return None
  ...
  def end_coroutine(self, use_time: float
    ...
    try:
      ...
      if overwrite_time_resources is not None:
        use_time, use_resources = overwrite_time_resources
      self.future_buffer[pos].set_result(result)
    except RateLimitError as e:
      use_time = e.use_time
      use_resources = e.use_resources
